...
klagenfurt_bbox <- st_read("BB_large.shp")
...
raster_cropped <- crop(raster_stack, klagenfurt_bbox)

# Plot each band individually
par(mfrow = c(2, 2))  # Set layout to 2x2 for better overview

plot(raster_cropped$Green, main = "Green Band (B3)", col = terrain.colors(10))
...
savi<- ((raster_cropped$NIR - raster_cropped$Red)*(1+0.5))/(raster_cropped$NIR + raster_cropped$Red+0.5)
...
ibi<- ((2*raster_cropped$SWIR/(raster_cropped$SWIR+raster_cropped$NIR))-((raster_cropped$NIR/(raster_cropped$NIR+raster_cropped$Red))+(raster_cropped$Green/(raster_cropped$Green+raster_cropped$SWIR))))/((2*raster_cropped$SWIR/(raster_cropped$SWIR+raster_cropped$NIR))+((raster_cropped$NIR/(raster_cropped$NIR+raster_cropped$Red))+(raster_cropped$Green/(raster_cropped$Green+raster_cropped$SWIR))))
#ibi[ibi < -1 | ibi > 1] <- NA
# Plot IBI
...
# =====================================================
# 5. BARE SOIL INDEX (BI)
# =====================================================
#Formula: ((SWIR1+Red)-(NIR+Blue))/((SWIR1+Red)+(NIR+Blue))

bi<-((raster_cropped$SWIR+raster_cropped$Red)-(raster_cropped$NIR+raster_cropped$Blue))/((raster_cropped$SWIR+raster_cropped$Red)+(raster_cropped$NIR+raster_cropped$Blue))

plot(bi, main = "BI - Klagenfurt")

writeRaster(bi, "BI_Klagenfurt.tif", overwrite = TRUE)
# =====================================================
# 5. CALCULATE Modified Normalized Difference Water Index (MNDWI)
# =====================================================
#MNDWI Formula: (Green-SWIR)/(Green+SWIR)

#mndwi<-(raster_cropped$Green-raster_cropped$SWIR)/(raster_cropped$Green+raster_cropped$SWIR)
...
thermal_urban <- terra::mask(raster_cropped$Thermal, urban_mask)
...
thermal_veg <- terra::mask(raster_cropped$Thermal, veg_mask)
...
suhi_raster <- (raster_cropped$Thermal - mean_LST) / std_LST
